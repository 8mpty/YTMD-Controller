...
const createQueueItemStyles = (itemWidth, itemHeight, dimensions) => StyleSheet.create({
  queueItem: {
    alignItems: "center",
...
const QueueItem = memo(({
  ...
  const { videoId, title, imgSrc } = useMemo(() => ({
    ...
    imgSrc: item.playlistPanelVideoRenderer.thumbnail.thumbnails.slice(-1)[0].url,
  }), [item]);
      ...
      >
        <Image
          ...
          defaultSource={Platform.select({
            web: { uri: 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7' },
            default: null
          })}
        />
        <TouchableOpacity
  ...
  return (
    ...
    prevProps.likedTracks[prevProps.item.playlistPanelVideoRenderer.videoId] === 
    nextProps.likedTracks[nextProps.item.playlistPanelVideoRenderer.videoId] &&
...
const NowQueue = memo(({
  ...
  const renderItem = useCallback(({ item, index }) => (
    ...
    />
  ), [currentVideoId, likedTracks, onLikeToggle, handleTrackSelect, ITEM_WIDTH, ITEM_HEIGHT, dimensions]);

  const keyExtractor = useCallback(
    item => item.playlistPanelVideoRenderer.videoId || Math.random().toString(36).substring(7),
    []
  );
